...
public class WordTranscriber 
{
    public class Phonetic
    {
        ...
        public Tuple<string[],int> get_phonetic()
        {
            ...
            return new Tuple<string[], int>(sounds.ToArray(), k);
        }
    ...
    public class Letter
    {
        ...
        private List<(string, string)> sonorus_deaf_pairs = new List<(string, string)>
            ...
            if (prev_letter != null)
            {
                if ((prev_letter.GetType() != typeof(Letter)))
                {
                    throw new Exception(String.Format(
                        "Предыдущая буква должна быть объектом класса {0!r}, или None (передан тип {1!r}).",
                        this.GetType(), prev_letter.GetType()));
                }
            }
